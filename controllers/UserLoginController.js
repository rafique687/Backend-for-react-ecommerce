...
exports.userwhishlist= async(req,res) => {
    ...
    const {item_id} =req.body;
     ...
     const whishlists = new whishlist({
        userid : userId,
        productid: item_id,
    });
     await whishlists.save();
     console.log(whishlists);

     return res.status(200).json({ success : true ,message:'Item Added to whishlist',data:whishlists});
 } 
 catch(error){
...
exports.getWishlist = async (req, res) => {
    try {
        ...
        const wishlistItem = await whishlist
            .find({ userid: userid })
            .populate("productid");
